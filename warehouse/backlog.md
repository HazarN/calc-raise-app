...
| CR-0009 | Haftalık yayılım / heat-map görünümü | Frontend (Web) | **Done (v0 demo)** — bkz. not aşağıda |
...
**Not (CR-0009):** Orijinal istekte "kolonlar gün, hücrede sadece egzersiz adı" deniyordu; ben analiz dokümanı §4'te "satırlar kas grubu" değişikliğini önermiştim ama bu konuda açık bir onay alınmamıştı. Çelişkiyi çözmek için ikisini birleştirdim: **kolonlar hâlâ haftanın 7 günü, satırlar kas grubu** — hücrede hem egzersiz adları hem set sayısı var (orijinal istek), hücre rengi o günkü hacme göre yoğunlaşıyor ve aynı kas grubu 48 saatten (2 günden) az arayla tekrar edince kırmızı çerçeveyle işaretleniyor (analiz önerisi — asıl "yeterince dinlendi mi" amacını satır=kas grubu olmadan görsel olarak çözmek zordu). Rest-kontrolü v0'da yalnızca **hafta içi** günler arasında yapılıyor, hafta sınırı ötesi (geçen haftanın son günü - bu haftanın ilk günü) kapsam dışı. Backend: `GET /api/workout-weeks/{id}/weekly-spread`, canlı Neon DB'ye karşı doğrulandı (bir EF Core sorgu çeviri hatası bulunup düzeltildi — LINQ projeksiyonunu anonim tipe çekip materialize ettikten sonra filtrelemek gerekti). Web: iki view arası basit bir sekme (tab) geçişi eklendi, ayrı bir router kütüphanesi eklenmedi.
